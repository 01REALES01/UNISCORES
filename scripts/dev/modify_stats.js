...
const path = 'src/modules/matches/components/match-stats.tsx';
let data = fs.readFileSync(path, 'utf8');

// Replace everything inside MatchStats function
const replacement = `export function MatchStats({ match, eventos, sportName }: MatchStatsProps) {
    ...
    const stats = useMemo(() => {
        const teamA = { goals: 0, fouls: 0, yellowCards: 0, redCards: 0, pts1: 0, pts2: 0, pts3: 0, players: {} as Record<string, { points: number, goals: number, pts1: number, pts2: number, pts3: number, profile: any }> };
        const teamB = { goals: 0, fouls: 0, yellowCards: 0, redCards: 0, pts1: 0, pts2: 0, pts3: 0, players: {} as Record<string, { points: number, goals: number, pts1: number, pts2: number, pts3: number, profile: any }> };

        let mvp: any = null;
        let mvpPoints = 0;

        eventos.forEach((evRaw: any) => {
            const ev = evRaw;
            ...
            } else if (ev.tipo_evento === 'falta') team.fouls += 1;
            else if (ev.tipo_evento === 'tarjeta_amarilla') team.yellowCards += 1;
            else if (ev.tipo_evento === 'tarjeta_roja') team.redCards += 1;

            if (ev.jugadores && ev.jugadores.perfiles) {
                const pId = ev.jugadores.perfiles.id;
                if (!team.players[pId]) team.players[pId] = { points: 0, goals: 0, pts1: 0, pts2: 0, pts3: 0, profile: ev.jugadores.perfiles };
        ...
        const topScorersA = Object.values(teamA.players).filter(p => p.goals > 0 || p.points > 0).sort((a, b) => b.points - a.points);
        const topScorersB = Object.values(teamB.players).filter(p => p.goals > 0 || p.points > 0).sort((a, b) => b.points - a.points);
        const topContributors = [...Object.values(teamA.players), ...Object.values(teamB.players)].filter(p => p.points > 0).sort((a, b) => b.points - a.points).slice(0, 3);

        return { teamA, teamB, mvp, mvpPoints, topScorersA, topScorersB, topContributors };
    ...
    if (!hasEvents) {
        return (
            <div className="rounded-[2.5rem] bg-[#0A0705] border border-white/5 p-8 text-center mt-8">
                ...
                <h3 className="text-xs font-black uppercase tracking-[0.3em] text-white/30 font-outfit mb-2">Rendimiento Global</h3>
                <p className="text-[10px] text-white/20 font-bold max-w-sm mx-auto">No hay eventos registrados.</p>
            </div>
        );
    }
        ...
        <div className="rounded-[2.5rem] max-w-4xl mx-auto bg-gradient-to-b from-[#0A0705] to-[#040302] border border-white/5 p-10 mt-12 shadow-2xl relative overflow-hidden flex flex-col gap-12">
            ...
            <div className="relative z-10 w-full">
                <div className="flex items-center justify-between text-xs font-black uppercase tracking-widest text-white/60 mb-4 px-2">
                    ...
                    <span className="text-amber-500 px-4 flex-shrink-0 text-[10px] tracking-[0.3em]">{isBasketball ? 'PUNTOS' : 'ANOTACIONES'}</span>
            ...
            {/* Soccer Stats */}
            {isFootball && (
                <div className="relative z-10 grid grid-cols-2 gap-6 pt-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10 w-full pt-8 border-t border-white/5">
                ...
                {/* MVP Player Card */}
                {mvp && (
                    <div className="p-[2px] rounded-[2.5rem] bg-gradient-to-br from-amber-400 via-amber-600 to-amber-900 shadow-[0_0_50px_rgba(245,158,11,0.15)] relative group overflow-hidden">
                ...
                <div className="flex flex-col gap-6">
                    {/* Team A Scorers */}
                    <div className="bg-white/5 border border-white/5 rounded-[2rem] p-6 h-full flex flex-col">
                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40 mb-6 flex justify-between">
                            <span className="text-red-400">{match.equipo_a}</span>
                            <span>{isBasketball ? 'ESTADÍSTICAS' : 'GOLEADORES'}</span>
                        </p>
                        <div className="space-y-4 flex-1">
                                ...
                                <div key={idx} className="flex items-center justify-between border-b border-white/5 pb-3 last:border-0 last:pb-0">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex flex-col">
                                            ...
                                            {isBasketball && (
                                                <span className="text-[9px] font-black text-white/30 uppercase tracking-widest">{player.pts3} triples • {player.pts2} dobles • {player.pts1} t.libres</span>
                                            )}
                                        </div>
                                    </div>
                            ...
                            {topScorersA.length === 0 && <span className="text-xs font-bold text-white/20 italic">No hay anotadores registrados</span>}
                        </div>
                    </div>
                    {/* Team B Scorers */}
                    <div className="bg-white/5 border border-white/5 rounded-[2rem] p-6 h-full flex flex-col">
                        <p className="text-[9px] font-black uppercase tracking-[0.2em] text-white/40 mb-6 flex justify-between">
                            <span className="text-blue-400">{match.equipo_b}</span>
                            <span>{isBasketball ? 'ESTADÍSTICAS' : 'GOLEADORES'}</span>
                        </p>
                        <div className="space-y-4 flex-1">
                            ...
                            {topScorersB.length === 0 && <span className="text-xs font-bold text-white/20 italic">No hay anotadores registrados</span>}
                        </div>
                    </div>
                </div>
...
const startIndex = data.indexOf('export function MatchStats');
if (startIndex !== -1) {
    fs.writeFileSync(path, data.substring(0, startIndex) + replacement);
    console.log("Success");
} else {
    console.log("Failed to find MatchStats");
}
