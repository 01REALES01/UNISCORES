...
export function addPoints(
    ...
    if (deporte === 'Fútbol') {
        ...
        return addPointTenis(nuevo, equipo);
    } else {
        // Genérico
...
/**
 * Tenis/Tenis de Mesa: Añadir punto (juego directo simplificado)
 */
function addPointTenis(detalle: ScoreDetail, equipo: 'equipo_a' | 'equipo_b'): ScoreDetail {
    const setActual = detalle.set_actual || 1;
    ...
    // Verificar si ganó el set (6 juegos con 2 de diferencia, o 7-6 tiebreak)
    ...
    if ((juegosA >= 6 && juegosA - juegosB >= 2) || (juegosA === 7 && juegosB === 6)) {
        detalle.sets_a = (detalle.sets_a || 0) + 1;
        if (setActual < 3) detalle.set_actual = setActual + 1;
...
/**
 * Obtener marcador actual para mostrar (con sub-scores para el período actual)
 */
export function getCurrentScore(deporte: string, detalle: ScoreDetail): ScoreResult {
    if (deporte === 'Fútbol') {
        ...
        return {
            ...
            extra: `Q${cuarto}`,
            subLabel: `Pts Q${cuarto}`
        };
    } else if (deporte === 'Voleibol') {
        const set = detalle.set_actual || 1;
        return {
            scoreA: detalle.sets_a || 0,
            scoreB: detalle.sets_b || 0,
            subScoreA: detalle.sets?.[set]?.puntos_a || 0,
            subScoreB: detalle.sets?.[set]?.puntos_b || 0,
            extra: `Set ${set}`,
            subLabel: `Pts Set ${set}`
        };
    } else if (deporte === 'Tenis' || deporte === 'Tenis de Mesa') {
        const set = detalle.set_actual || 1;
        return {
            scoreA: detalle.sets_a || 0,
            scoreB: detalle.sets_b || 0,
            subScoreA: detalle.sets?.[set]?.juegos_a || 0,
            subScoreB: detalle.sets?.[set]?.juegos_b || 0,
            extra: `Set ${set}`,
            subLabel: `Juegos Set ${set}`
        };
    } else {
        return {
            scoreA: detalle.total_a || 0,
