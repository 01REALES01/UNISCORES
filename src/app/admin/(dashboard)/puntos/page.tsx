...
function ClasificacionTab() {
    ...
    const handleCalcular = async () => {
        ...
        try {
            const res = await fetch('/api/admin/calcular-posiciones', {
                ...
                body: JSON.stringify({ disciplina_id: selectedDisc, genero: selectedGenero, categoria: selectedCategoria }),
            });
            if (res.ok) {
                toast.success('Posiciones calculadas');
                fetchData(selectedDisc, selectedGenero, selectedCategoria);
            }
        } finally {
    ...
    const handleDelete = async (entryId: number) => {
        await supabase.from('clasificacion_disciplina').delete().eq('id', entryId);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-wrap gap-3">
                <select value={selectedDisc ?? ''} onChange={e => setSelectedDisc(Number(e.target.value) || null)} className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm">
                    <option value="">Selecciona disciplina...</option>
                    {disciplinas.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
                </select>
                <select value={selectedGenero} onChange={e => setSelectedGenero(e.target.value as any)} className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm">
                    {GENEROS.map(g => <option key={g} value={g}>{g}</option>)}
                </select>
                {hasCategoria && (
                    <select value={selectedCategoria ?? ''} onChange={e => setSelectedCategoria(e.target.value as any)} className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-white text-sm">
                ...
                <>
                    <div className="rounded-2xl border border-white/5 bg-white/[0.02] overflow-hidden">
                        <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
                            ...
                            {hasBracket && (
                                <button onClick={handleCalcular} disabled={calculating} className="text-xs text-violet-400 bg-violet-500/10 px-3 py-1 rounded-lg">
                                    {calculating ? 'Calculando...' : 'Calcular desde brackets'}
                                </button>
                            )}
                        </div>
                        <div className="divide-y divide-white/5">
                            {Array.from({ length: maxPositions }, (_, i) => i + 1).map(pos => (
                                <div key={pos} className="flex items-center gap-4 px-4 py-3">
                                    <span className="text-xs font-black text-white/30 w-8">{pos}°</span>
                                    <select value={positions[pos] ?? ''} onChange={e => setPositions(prev => ({ ...prev, [pos]: e.target.value || null }))} className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-1.5 text-white text-sm">
                                        <option value="">Sin asignar</option>
                                        {dropdownOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                                    </select>
                                    <span className="text-amber-400/70 text-xs font-bold w-12 text-right">{getPuntos(pos)} pts</span>
                                </div>
                            ))}
                        </div>
                    </div>
                    <button onClick={handleSave} disabled={saving} className="w-full py-3 rounded-2xl bg-amber-500/10 border border-amber-500/20 text-amber-400 font-bold">
            ...
            {existing.length > 0 && (
                <div className="space-y-2">
                    <p className="text-xs text-white/30 uppercase font-black">Entradas Guardadas</p>
                    {existing.map(e => (
                        <div key={e.id} className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] border border-white/5">
                            <div className="flex items-center gap-3">
                                <span className="text-amber-400 font-black text-sm">{e.posicion}°</span>
                                <span className="text-white/80 text-sm">{(e.carreras as any)?.nombre}</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <span className="text-amber-400/60 text-xs">{e.puntos_obtenidos} pts</span>
                                <button onClick={() => handleDelete(e.id)} className="text-white/20 hover:text-rose-400 transition-colors"><Trash2 size={14} /></button>
                            </div>
                        </div>
                    ))}
                </div>
...
function FairPlayAdminTab() {
    ...
    const loadData = useCallback(async (discId: number, genero: string) => {
        ...
        (pts ?? []).forEach((p: any) => {
            [p.delegacion_a || p.equipo_a, p.delegacion_b || p.equipo_b].forEach(t => {
                            ...
                            <div className="flex flex-wrap gap-3">
                                ...
                                <div className="flex flex-col gap-1">
                                    ...
                                    >
                                        ...
                                        {partidos.map(p => (
                                            <option key={p.id} value={p.id}>{p.equipo_a} vs {p.equipo_b}</option>
                                        ))}
                                    </select>
                                </div>
                            </div>
