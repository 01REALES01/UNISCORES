...
export interface SuggestiveSearchProps {
    ...
    /** Add the placeholder functionality externally. Optional initial search query */
    value?: string;
}
...
export const SlideEffect: React.FC<EffectRendererProps> = ({
    ...
    if (prefersReducedMotion) {
        ...
        return (
            <span className="text-sm text-slate-500 select-none">{text}</span>
        );
    }
...
export const SuggestiveSearch: React.FC<SuggestiveSearchProps> = ({
    ...
    className,
    Leading = () => <Search className="size-4 text-slate-500" />,
    showLeading = true,
    Trailing = () => <Search className="size-4 text-slate-500" />,
    showTrailing = false,
    effect = "typewriter",
    EffectComponent,
    typeDurationMs = 500,
    ...
    return (
        <div
            ref={wrapperRef}
            className={cn(
                "relative flex items-center gap-x-2 w-full",
                className
            )}
            style={{ maxWidth: "100%" }}
        >
            {/* Leading icon (optional) */}
            <div ref={leadingRef} className="flex-shrink-0 absolute left-4 z-10 pointers-events-none">
                {showLeading && <Leading />}
            </div>
            ...
            <input
                ...
                className="w-full h-full bg-transparent outline-none text-sm text-white placeholder:text-transparent pl-12 pr-4 z-10"
                placeholder=""
                aria-label="search"
            ...
            <div ref={trailingRef} className="flex-shrink-0 absolute right-4 z-10">
            ...
            {overlayActive && (
                <div
                    ...
                    style={{
                        position: "absolute",
                        left: "3rem", // adjusted for our custom padding
                        right: rightOffsetPx != null ? `${rightOffsetPx}px` : "1rem",
                        top: 0,
                        bottom: 0,
                        display: "flex",
                        alignItems: "center",
