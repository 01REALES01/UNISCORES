...
-- niveles en Tenis y Tenis de Mesa.
-- Valores: 'intermedio' | 'avanzado' | NULL
-- Natación NO usa categoría (se diferencia por prueba: estilo + distancia)
-- ─────────────────────────────────────────────────────────────────────────────

ALTER TABLE public.partidos
    ADD COLUMN IF NOT EXISTS categoria TEXT
        CHECK (categoria IN ('intermedio', 'avanzado'));

COMMENT ON COLUMN public.partidos.categoria IS
    'Nivel del partido. Aplica solo a Tenis y Tenis de Mesa (intermedio/avanzado). NULL para otros deportes.';
